...
class CNN_AutoEncoder_01:	
	...
	def network(self, network_input=None):
		...
		# Flatten
		#flat = self.flatten(encode5, 'Flatten')
		#if self.verbose: print('{} -> {},\t\tLayer Output Shape:\t{}'.format('Encode5', 'Flatten', flat.get_shape().as_list()))
		...
		if self.verbose: print('{} -> {},\t\tLayer Output Shape:\t{}'.format('Flatten', 'Decode1', decode1.get_shape().as_list()))
		...
		decode5 = self.decoder_layer(layer_input=decode4, is_flat=False, last_img_shape=None, n_filters=NUM_NN_IMAGE_OUTPUTS, activation=tf.nn.relu, unpooling_size=[480, 640], trainable=True, name='Decoder__L5')
		if self.verbose: print('{} -> {},\t\tLayer Output Shape:\t{}'.format('Encode4', 'Decode5', decode5.get_shape().as_list()))

		return tf.nn.sigmoid(decode5 / tf.reduce_mean(decode5))
